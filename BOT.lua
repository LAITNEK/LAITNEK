...
local getversion = https.request('https://api.th3LAITNEK.com/version/')
...
#!/usr/bin/env bash
cd $(cd $(dirname $0); pwd)
if ! screen -list | grep -q "LAITNEK"; then
screen -S LAITNEK lua ./inc/BOT.lua
fi
screen -X -S LAITNEK kill
screen -S LAITNEK lua ./inc/BOT.lua
]])
file:close()
...
_info = load_config()
LAITNEK = _info.TOKEN:match("(%d+)")
our_id = tonumber(LAITNEK)
bot_username = redis:get(LAITNEK..":username")
...
print('\n\27[0;32mPlugins is : '..#list_plug..' Are Active.\n\n\27[0;31mThe SOURCE RUN IS TH3LAITNEK v'..redis:get(LAITNEK..':VERSION')..'...\n\27[m')
end
load_plugins()
...
return false end 
if not (tostring(msg.chat_id_):match('^-100') or tostring(msg.chat_id_):match('-')) and redis:get(LAITNEK..':mute_pv:'..msg.sender_user_id_) then
...
if redis:get(LAITNEK..'mute_text'..msg.chat_id_) then --قفل الدردشه
