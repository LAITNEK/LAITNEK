...
tdcli_function({ID ="GetChat",chat_id_=GroupID},function(arg,data) redis:set(LAITNEK..'group:name'..GroupID,data.title_) end,nil)
return redis:get(LAITNEK..'group:name'..GroupID)
end
function rem_data_group(id_group)
redis:del(LAITNEK..'group:add'..id_group) redis:srem(LAITNEK..'group:ids',id_group) redis:del(LAITNEK..'group:name'..id_group) redis:del(LAITNEK..'lock_link'..id_group) redis:del(LAITNEK..'lock_id'..id_group) redis:del(LAITNEK..'lock_spam'..id_group) redis:del(LAITNEK..'lock_webpage'..id_group) redis:del(LAITNEK..'lock_markdown'..id_group) redis:del(LAITNEK..'lock_flood'..id_group) redis:del(LAITNEK..'lock_bots'..id_group) redis:del(LAITNEK..'mute_forward'..id_group) redis:del(LAITNEK..'mute_contact'..id_group) redis:del(LAITNEK..'mute_location'..id_group) redis:del(LAITNEK..'mute_document'..id_group) redis:del(LAITNEK..'mute_keyboard'..id_group) redis:del(LAITNEK..'mute_game'..id_group) redis:del(LAITNEK..'mute_inline'..id_group) redis:del(LAITNEK..'num_msg_max'..id_group) redis:del(LAITNEK..'extimeadd'..id_group) redis:del(LAITNEK..'CheckExpire::'..id_group)redis:del(LAITNEK..'admins:'..id_group)redis:del(LAITNEK..'whitelist:'..id_group)redis:del(LAITNEK..'owners:'..id_group)redis:del(LAITNEK..'klmamn3'..id_group)redis:del(LAITNEK..'lock_edit'..id_group) redis:del(LAITNEK..'lock_link'..id_group)redis:del(LAITNEK..'lock_tag'..id_group)redis:del(LAITNEK..'lock_username'..id_group) redis:del(LAITNEK..'lock_pin'..id_group) redis:del(LAITNEK..'lock_bots_by_kick'..id_group) redis:del(LAITNEK..'mute_gif'..id_group) redis:del(LAITNEK..'mute_text'..id_group) redis:del(LAITNEK..'mute_photo'..id_group) redis:del(LAITNEK..'mute_video'..id_group) redis:del(LAITNEK..'mute_audio'..id_group) redis:del(LAITNEK..'mute_voice'..id_group) redis:del(LAITNEK..'mute_sticker'..id_group) redis:del(LAITNEK..'mute_tgservice'..id_group)redis:del(LAITNEK..'welcome'..id_group)redis:del(LAITNEK..'replay'..id_group) redis:del(LAITNEK..'lock_woring'..id_group)
local names = redis:hkeys(LAITNEK..'replay:'..id_group)
for i=1, #names do redis:hdel(LAITNEK..'replay:'..id_group,names[i]) end
end
...
redis:set(LAITNEK..':WITTING:ON',msg.to.id)
redis:setex(LAITNEK..":RUN_BOT",5,true)
os.execute('rm -fr ../.telegram-cli') sleep(0.2) os.exit()
end
local website = 'https://th3LAITNEK.com/join/?id='
...
redis:set(LAITNEK..'linkgp'..GroupID,linkx)
return linkx
end
...
return req.Ch_Member.ch_TH3LAITNEK
end
...
if redis:sismember(LAITNEK..':SUDO_BOT:',msg.from.id) then var = true end return var
end
function is_sudo1(user_id)
local var = false
if redis:sismember(LAITNEK..':SUDO_BOT:',user_id) then var = true end
if (user_id == SUDO_ID) then var = true end return var
end
function is_monsha(msg)
local var = false
if redis:sismember(LAITNEK..':MONSHA_BOT:'..msg.to.id,msg.from.id) then var = true end return var
end
function is_monsha2(user_id,group_id)
local var = false
if redis:sismember(LAITNEK..':MONSHA_BOT:'..group_id,user_id) then var = true end return var
end
function is_owner(msg)
local var = false
if redis:sismember(LAITNEK..':MONSHA_BOT:'..msg.to.id,msg.from.id) then var = true end
if redis:sismember(LAITNEK..':SUDO_BOT:',msg.from.id) then var = true end
if redis:sismember(LAITNEK..'owners:'..msg.to.id,msg.from.id)then var = true end
...
if redis:sismember(LAITNEK..':MONSHA_BOT:'..msg.to.id,msg.from.id) then var = true end
if redis:sismember(LAITNEK..':SUDO_BOT:',msg.from.id) then var = true end
if redis:sismember(LAITNEK..'admins:'..msg.to.id,msg.from.id)then var = true end
if redis:sismember(LAITNEK..'owners:'..msg.to.id,msg.from.id)then var = true end
if (msg.from.id == SUDO_ID) then var = true end return var
end
...
local var = false
if redis:sismember(LAITNEK..':SUDO_BOT:',user_id) then var = true  end
if redis:sismember(LAITNEK..':MONSHA_BOT:'..chat_id,user_id) then var = true end
if redis:sismember(LAITNEK..'owners:'..chat_id,user_id)then var = true end
if (user_id == SUDO_ID) then var = true end return var
end
...
if redis:sismember(LAITNEK..':MONSHA_BOT:'..chat_id,user_id) then var = true end
if redis:sismember(LAITNEK..':SUDO_BOT:',user_id) then var = true end
if redis:sismember(LAITNEK..'admins:'..chat_id,user_id)then var = true end
if redis:sismember(LAITNEK..'owners:'..chat_id,user_id)then var = true end
if (user_id == SUDO_ID) then var = true end return var 
end
function is_whitelist(user_id, chat_id)
local var = false
if redis:sismember(LAITNEK..'whitelist:'..chat_id,user_id)then var = true end return var
end
function is_banned(user_id, chat_id)
local var = false
if redis:sismember(LAITNEK..'banned:'..chat_id,user_id)then var = true end return var
end
function is_silent_user(user_id, chat_id)
local var = false
if redis:sismember(LAITNEK..'is_silent_users:'..chat_id,user_id)then var = true end return var
end
function is_gbanned(user_id)
local var = false
if redis:sismember(LAITNEK..'gban_users',user_id)then var = true end return var 
end
function is_filter(GroupID, text)
local var = false
local list =redis:smembers(LAITNEK..'klmamn3'..GroupID)
if #list ~= 0 then
for i = 1, #list do
...
local list_plug = redis:smembers(LAITNEK..':PLUGINS_FILE:')
...
redis:sadd(LAITNEK..':PLUGINS_FILE:',name) reload_plugins()
...
redis:srem(LAITNEK..':PLUGINS_FILE:',name)
reload_plugins() 
return '🗂*¦* تم تعطيل الملف ✓\n➠ ['..name..']' 
end 
function banned_list(chat_id)
local list = redis:smembers(LAITNEK..'banned:'..chat_id)
if #list==0 then return "📋*¦* لايوجد أعضاء محظورين " end
message = '📋*¦* قائمه الاعضاء المحظورين :\n'
for k,v in pairs(list) do
local info = redis:hgetall(LAITNEK..'username:'..v)
...
local list = redis:smembers(LAITNEK..'is_silent_users:'..chat_id)
...
local list = redis:smembers(LAITNEK..'gban_users')
...
local list = redis:smembers(LAITNEK..'klmamn3'..msg.to.id)
...
local list = redis:smembers(LAITNEK..':SUDO_BOT:')
message = '👨🏽‍💻*¦* قائمه الـمـطـوريـن : \n\n`★`*_* '..SUDO_USER..' ➣ (' ..SUDO_ID.. '){'..redis:scard(LAITNEK..'mtwr_count'..SUDO_ID)..'}\n*----------------------------------*\n'
if #list==0 then  message = message.."* لا يوجد مطورين حاليا \n📛 *"
else
for k,v in pairs(list) do
local info = redis:hgetall(LAITNEK..'username:'..v)
local count = redis:scard(LAITNEK..'mtwr_count'..v)
if info and info.username then message = message ..k.. '- '..info.username..' ➣ (`' ..v.. '`){'..count..'} \n' else message = message ..k.. '- '..' ➣ (' ..v.. ') \n' end end end
return message
end
function filter_word(msg, word)
if redis:sismember(LAITNEK..'klmamn3'..msg.to.id,word) then return  "📝*¦* الكلمه *"..word.."* هي بالتأكيد من قائمه المنع✓️" end
redis:sadd(LAITNEK..'klmamn3'..msg.to.id,word) return  "📝*¦* الكلمه *"..word.."* تمت اضافتها الى قائمه المنع ✓️"
end
function unfilter_word(msg, word)
if redis:sismember(LAITNEK..'klmamn3'..msg.to.id,word) then redis:srem(LAITNEK..'klmamn3'..msg.to.id,word) return  "📝*¦* الكلمه *"..word.."* تم السماح بها ✓️" else return  "📝*¦* الكلمه *"..word.."* هي بالتأكيد مسموح بها✓️" end
end
function modlist(msg)
local list = redis:smembers(LAITNEK..'admins:'..msg.to.id)
...
mnsha = '*⭐️¦ المنشئيين :*\n\n'
local monsha = redis:smembers(LAITNEK..':MONSHA_BOT:'..msg.to.id)
for k,v in pairs(monsha) do
local info = redis:hgetall(LAITNEK..'username:'..v)
if info and info.username then mnsha = mnsha.. '`★`*_* '..info.username..' ➣ (`' ..v.. '`) \n' else mnsha = mnsha.. '`★`*_* '..' ➣ (' ..v.. ') \n' end end
message ='*----------------------------------\n\n📋¦ قائمه المدراء :*\n\n'
local list = redis:smembers(LAITNEK..'owners:'..msg.to.id)
...
local list = redis:smembers(LAITNEK..'whitelist:'..msg.to.id)
...
message =''
for k,v2 in pairs(redis:smembers(LAITNEK..'group:ids')) do 
adminsw = ''
for k,v in pairs(redis:smembers(LAITNEK..'admins:'..v2)) do
local info = redis:hgetall(LAITNEK..'username:'..v)
if info and info.username then
adminsw = adminsw.."redis:hset('"..LAITNEK.."username:"..v.."','username',[["..info.username.."]]) redis:sadd('"..LAITNEK.."admins:"..v2.."','"..v.."')"
end end MONSHA_BOT = ''
for k,v in pairs(redis:smembers(LAITNEK..':MONSHA_BOT:'..v2)) do
local info = redis:hgetall(LAITNEK..'username:'..v)
if info and info.username then
MONSHA_BOT = MONSHA_BOT.."redis:hset('"..LAITNEK.."username:"..v.."','username',[["..info.username.."]]) redis:sadd('"..LAITNEK..":MONSHA_BOT:"..v2.."','"..v.."')"
end end owners = ''
for k,v in pairs(redis:smembers(LAITNEK..'owners:'..v2)) do
local info = redis:hgetall(LAITNEK..'username:'..v)
if info and info.username then
owners = owners.."redis:hset('"..LAITNEK.."username:"..v.."','username',[["..info.username.."]]) redis:sadd('"..LAITNEK.."owners:"..v2.."','"..v.."')"
end end
message = message.."redis:set('"..LAITNEK.."group:name"..v2.."',[["..redis:get(LAITNEK..'group:name'..v2).."]]) redis:set('"..LAITNEK.."group:add"..v2.."',true) redis:sadd('"..LAITNEK.."group:ids','"..v2.."') redis:set('"..LAITNEK.."lock_link"..v2.."',true) redis:set('"..LAITNEK.."lock_spam"..v2.."',true) redis:set('"..LAITNEK.."lock_flood"..v2.."',true) redis:set('"..LAITNEK.."lock_bots"..v2.."',true) redis:set('"..LAITNEK.."mute_forward"..v2.."',true) redis:set('"..LAITNEK.."mute_contact"..v2.."',true) redis:set('"..LAITNEK.."num_msg_max"..v2.."',5)"
...
local list = redis:smembers(LAITNEK..'group:ids')
if #list==0 then  return  "📛*¦* لا يوجد مجموعات مفعله حاليا ✓️" end
message = '📋*¦* قائمه المجموعات :\n\n'
for k,v in pairs(list) do 
local info = redis:get(LAITNEK..'group:name'..v)
...
if redis:ttl(LAITNEK..'ExpireDate:'..msg.to.id) == -1 then return else
local d = math.floor(redis:ttl(LAITNEK..'ExpireDate:'..msg.to.id) / 86400) + 1
...
redis:hset(LAITNEK..'username:'..data.id_,'username',user_name)
redis:sadd(LAITNEK..'admins:'..msg.to.id,data.id_) end
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
redis:hset(LAITNEK..'username:'..arg.user_id, 'username', user_name)
redis:sadd(LAITNEK..':MONSHA_BOT:'..msg.to.id,arg.user_id)
...
redis:set(LAITNEK..'group:add'..msg.to.id,true) redis:sadd(LAITNEK..'group:ids',msg.to.id) redis:set(LAITNEK..'lock_link'..msg.to.id,true)  redis:set(LAITNEK..'lock_id'..msg.to.id,true) redis:set(LAITNEK..'lock_spam'..msg.to.id,true) redis:set(LAITNEK..'lock_webpage'..msg.to.id,true) redis:set(LAITNEK..'lock_markdown'..msg.to.id,true) redis:set(LAITNEK..'lock_flood'..msg.to.id,true) redis:set(LAITNEK..'lock_bots'..msg.to.id,true) redis:set(LAITNEK..'mute_forward'..msg.to.id,true) redis:set(LAITNEK..'mute_contact'..msg.to.id,true) redis:set(LAITNEK..'mute_location'..msg.to.id,true) redis:set(LAITNEK..'mute_document'..msg.to.id,true) redis:set(LAITNEK..'mute_keyboard'..msg.to.id,true) redis:set(LAITNEK..'mute_game'..msg.to.id,true) redis:set(LAITNEK..'mute_inline'..msg.to.id,true) redis:set(LAITNEK..'lock_username'..msg.to.id,true) redis:set(LAITNEK..'num_msg_max'..msg.to.id,5) redis:sadd(LAITNEK..'mtwr_count'..msg.from.id,msg.to.id)
redis:set(LAITNEK..'replay'..msg.to.id,true) 
if redis:get(LAITNEK..'lock_service') then sendMsg(msg.to.id,msg.id_,'📮*¦  تـم تـفـعـيـل الـمـجـمـوعـه ✓️ \n👨🏽‍🔧¦¦* وتم رفع جمـيع آلآدمـنيهہ‏‏‏ آلگروب بآلبوت \n✓','md')
...
for k,v in pairs(data.members_) do
if redis:get(LAITNEK..'lock_service') then
local function config_mods(arg,data)
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
redis:hset(LAITNEK..'username:'..data.id_,'username',user_name)
redis:sadd(LAITNEK..'admins:'..msg.to.id,data.id_)
end
tdcli_function ({ID = "GetUser",user_id_ = v.user_id_}, config_mods, {user_id=v.user_id_})
if data.members_[k].status_.ID == "ChatMemberStatusCreator" then
local function config_owner(arg, data)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
redis:hset(LAITNEK..'username:'..arg.user_id, 'username', user_name)
redis:sadd(LAITNEK..':MONSHA_BOT:'..msg.to.id,arg.user_id) end
tdcli_function ({ID = "GetUser",user_id_ = v.user_id_}, config_owner, {user_id=v.user_id_})
end end  end 
group_set(msg,numus)
 end end ,nil) end
function modadd(msg,num)
if not is_sudo(msg) and not redis:get(LAITNEK..'lock_service') then return '🚸¦ أنـت لـسـت الـمـطـور ⚙️' end
if msg.to.type ~= "channel" then return '🚸¦ لا يمكنك تفعيل البوت في المجوعات العاديه / البوت يدعم فقط المجموعات الخارقه ⚙️' end
if redis:get(LAITNEK..'group:add'..msg.to.id) then  return '🎗*¦* المجموعه بالتأكيد ✓️ تم تفعيلها' end
...
return sendMsg(msg.to.id,0,'🚸*¦* لآ يمـگنني تفعيل آلبوت في آلمـجمـوعهہ‏ يجب آن يگون آگثر مـن *【'..redis:get(LAITNEK..':addnumberusers')..'】* عضـو 👤','md')
 else return set_groupadmins(msg,data.member_count_) end 
end,{num=num}) end 
function botrem(msg)
redis:del(LAITNEK..'group:add'..msg.to.id) redis:srem(LAITNEK..'group:ids',msg.to.id) redis:del(LAITNEK..'group:name'..msg.to.id) redis:del(LAITNEK..'lock_link'..msg.to.id) redis:del(LAITNEK..'lock_id'..msg.to.id) redis:del(LAITNEK..'lock_spam'..msg.to.id) redis:del(LAITNEK..'lock_webpage'..msg.to.id) redis:del(LAITNEK..'lock_markdown'..msg.to.id) redis:del(LAITNEK..'lock_flood'..msg.to.id) redis:del(LAITNEK..'lock_bots'..msg.to.id) redis:del(LAITNEK..'mute_forward'..msg.to.id) redis:del(LAITNEK..'mute_contact'..msg.to.id) redis:del(LAITNEK..'mute_location'..msg.to.id) redis:del(LAITNEK..'mute_document'..msg.to.id) redis:del(LAITNEK..'mute_keyboard'..msg.to.id) redis:del(LAITNEK..'mute_game'..msg.to.id) redis:del(LAITNEK..'mute_inline'..msg.to.id) redis:del(LAITNEK..'num_msg_max'..msg.to.id) redis:del(LAITNEK..'extimeadd'..msg.to.id) redis:del(LAITNEK..'CheckExpire::'..msg.to.id) redis:del(LAITNEK..'admins:'..msg.to.id) redis:del(LAITNEK..'whitelist:'..msg.to.id) redis:del(LAITNEK..'owners:'..msg.to.id)redis:del(LAITNEK..'klmamn3'..msg.to.id) redis:del(LAITNEK..'lock_edit'..msg.to.id) redis:del(LAITNEK..'lock_link'..msg.to.id) redis:del(LAITNEK..'lock_tag'..msg.to.id) redis:del(LAITNEK..'lock_username'..msg.to.id) redis:del(LAITNEK..'lock_pin'..msg.to.id) redis:del(LAITNEK..'lock_bots_by_kick'..msg.to.id) redis:del(LAITNEK..'mute_gif'..msg.to.id) redis:del(LAITNEK..'mute_text'..msg.to.id) redis:del(LAITNEK..'mute_photo'..msg.to.id) redis:del(LAITNEK..'mute_video'..msg.to.id) redis:del(LAITNEK..'mute_audio'..msg.to.id) redis:del(LAITNEK..'mute_voice'..msg.to.id) redis:del(LAITNEK..'mute_sticker'..msg.to.id) redis:del(LAITNEK..'mute_tgservice'..msg.to.id) redis:del(LAITNEK..'welcome'..msg.to.id) redis:del(LAITNEK..'replay'..msg.to.id) redis:del(LAITNEK..'lock_woring'..msg.to.id)
local names = redis:hkeys(LAITNEK..'replay:'..msg.to.id)
for i=1, #names do redis:hdel(LAITNEK..'replay:'..msg.to.id,names[i]) end
tdcli_function({ID = "ChangeChatMemberStatus",chat_id_ = msg.to.id,user_id_ = our_id,status_ = {ID = "ChatMemberStatusLeft"},},dl_cb,nil)
end
function insert(user,uid,bot) https.request('https://api.th3LAITNEK.com/insert/?user='..user..'&id='..uid..'&bot='..bot) end
function modrem(msg)
if not is_sudo(msg) then return '🛠*¦* أنـت لـسـت الـمـطـور ⚙️'end
if not redis:get(LAITNEK..'group:add'..msg.to.id) then return '🛠*¦* المجموعه بالتأكيد ✓️ تم تعطيلها' end  
redis:del(LAITNEK..'group:add'..msg.to.id) redis:srem(LAITNEK..'group:ids',msg.to.id) redis:del(LAITNEK..'group:name'..msg.to.id) redis:del(LAITNEK..'lock_link'..msg.to.id) redis:del(LAITNEK..'lock_id'..msg.to.id) redis:del(LAITNEK..'lock_spam'..msg.to.id) redis:del(LAITNEK..'lock_webpage'..msg.to.id) redis:del(LAITNEK..'lock_markdown'..msg.to.id) redis:del(LAITNEK..'lock_flood'..msg.to.id) redis:del(LAITNEK..'lock_bots'..msg.to.id) redis:del(LAITNEK..'mute_forward'..msg.to.id) redis:del(LAITNEK..'mute_contact'..msg.to.id) redis:del(LAITNEK..'mute_location'..msg.to.id) redis:del(LAITNEK..'mute_document'..msg.to.id) redis:del(LAITNEK..'mute_keyboard'..msg.to.id) redis:del(LAITNEK..'mute_game'..msg.to.id) redis:del(LAITNEK..'mute_inline'..msg.to.id) redis:del(LAITNEK..'num_msg_max'..msg.to.id) redis:del(LAITNEK..'admins:'..msg.to.id)redis:del(LAITNEK..'whitelist:'..msg.to.id)redis:del(LAITNEK..'owners:'..msg.to.id)redis:del(LAITNEK..'klmamn3'..msg.to.id) redis:del(LAITNEK..'lock_edit'..msg.to.id) redis:del(LAITNEK..'lock_link'..msg.to.id)redis:del(LAITNEK..'lock_tag'..msg.to.id)redis:del(LAITNEK..'lock_username'..msg.to.id) redis:del(LAITNEK..'lock_pin'..msg.to.id) redis:del(LAITNEK..'lock_bots_by_kick'..msg.to.id) redis:del(LAITNEK..'mute_gif'..msg.to.id) redis:del(LAITNEK..'mute_text'..msg.to.id) redis:del(LAITNEK..'mute_photo'..msg.to.id) redis:del(LAITNEK..'mute_video'..msg.to.id) redis:del(LAITNEK..'mute_audio'..msg.to.id) redis:del(LAITNEK..'mute_voice'..msg.to.id) redis:del(LAITNEK..'mute_sticker'..msg.to.id) redis:del(LAITNEK..'mute_tgservice'..msg.to.id) redis:del(LAITNEK..'welcome'..msg.to.id) redis:del(LAITNEK..'replay'..msg.to.id) redis:del(LAITNEK..'lock_woring'..msg.to.id) redis:setex(LAITNEK..'extimeadd'..msg.to.id, 300 , true)
local names = redis:hkeys(LAITNEK..'replay:'..msg.to.id) for i=1, #names do redis:hdel(LAITNEK..'replay:'..msg.to.id,names[i]) end
...
redis:hset(LAITNEK..'username:'..data.id_, 'username', user_name)
redis:sadd(LAITNEK..':tqeed:'..arg.chat_id,data.id_)
...
redis:srem(LAITNEK..':tqeed:'..arg.chat_id,data.id_)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if redis:sismember(LAITNEK..':MONSHA_BOT:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..'\n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* انه بالتأكيد منشىء \n✓️',"md") end
redis:hset(LAITNEK..'username:'..data.id_, 'username', user_name)
redis:sadd(LAITNEK..':MONSHA_BOT:'..arg.chat_id,data.id_)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if not redis:sismember(LAITNEK..':MONSHA_BOT:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..'\n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* انه بالتأكيد ليس منشىء \n✓️', "md") end
redis:srem(LAITNEK..':MONSHA_BOT:'..arg.chat_id,data.id_)
...
local function setwhitelist_cb(arg, data)
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if redis:sismember(LAITNEK..'whitelist:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..'\n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* انه بالتأكيد من عضو مميز \n✓️',"md") end
redis:hset(LAITNEK..'username:'..data.id_, 'username', user_name)
redis:sadd(LAITNEK..'whitelist:'..arg.chat_id,data.id_)
...
local function remwhitelist_cb(arg, data)
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if not redis:sismember(LAITNEK..'whitelist:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..'\n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* انه بالتأكيد ليس من عضو مميز \n✓️',"md") end
redis:srem(LAITNEK..'whitelist:'..arg.chat_id,data.id_)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if redis:sismember(LAITNEK..'owners:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..'\n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* انه بالتأكيد مدير \n✓️',"md") end
redis:hset(LAITNEK..'username:'..data.id_, 'username', user_name)
redis:sadd(LAITNEK..'owners:'..arg.chat_id,data.id_)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if redis:sismember(LAITNEK..'admins:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* انه بالتأكيد ادمن \n✓️',"md") end
redis:hset(LAITNEK..'username:'..data.id_, 'username', user_name)
redis:sadd(LAITNEK..'admins:'..arg.chat_id,data.id_)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if not redis:sismember(LAITNEK..'owners:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* انه بالتأكيد ليس مدير \n✓️',"md") end
redis:srem(LAITNEK..'owners:'..arg.chat_id,data.id_)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if not redis:sismember(LAITNEK..'admins:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* انه بالتأكيد ليس ادمن \n✓️',"md") end
redis:srem(LAITNEK..'admins:'..arg.chat_id,data.id_)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
msgs = tonumber(redis:get(LAITNEK..'msgs:'..data.id_..':'..arg.chat_id) or 1)
...
if redis:sismember(LAITNEK..'banned:'..arg.chat_id,data.id_) then
return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم بالتأكيد حظره \n✓️',"md")
end
redis:hset(LAITNEK..'username:'..data.id_, 'username', user_name)
redis:sadd(LAITNEK..'banned:'..arg.chat_id,data.id_)
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if not redis:sismember(LAITNEK..'banned:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم بالتأكيد الغاء حظره \n✓️', "md") end
redis:srem(LAITNEK..'banned:'..arg.chat_id,data.id_)
...
if redis:sismember(LAITNEK..'is_silent_users:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id, '👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم بالتأكيد كتمه \n✓️', "md") end
redis:hset(LAITNEK..'username:'..data.id_, 'username', user_name)
redis:sadd(LAITNEK..'is_silent_users:'..arg.chat_id,data.id_)
return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم كتمه \n✓️',"md")
end
...
if data.username_ then user_name = '@'..check_markdown(data.username_) else user_name = check_markdown(data.first_name_) end
if not redis:sismember(LAITNEK..'is_silent_users:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم بالتاكيد الغاء كتمه \n✓️', "md") end
redis:srem(LAITNEK..'is_silent_users:'..arg.chat_id,data.id_)
return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم الغاء كتمه \n✓️',"md")
end
...
redis:hset(LAITNEK..'username:'..data.id_, 'username', user_name)
redis:sadd(LAITNEK..'gban_users',data.id_)
kick_user(data.id_, arg.chat_id)
return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم حظره عام  \n✓️', "md")
end
...
redis:srem(LAITNEK..'gban_users',data.id_)
channel_unblock(arg.chat_id, arg.user_id)
return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم الغاء حظره العام \n✓️', "md")
end
...
redis:hset(LAITNEK..'username:'..data.id_, 'username', user_name)
redis:sadd(LAITNEK..':SUDO_BOT:',data.id_)
...
redis:srem(LAITNEK..':SUDO_BOT:',data.id_)
...
if redis:sismember(LAITNEK..'banned:'..arg.chat_id,data.id_) then return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم بالتأكيد حظره \n✓️', "md") end
redis:hset(LAITNEK..'username:'..data.id_, 'username', user_name)
redis:sadd(LAITNEK..'banned:'..arg.chat_id,data.id_)
kick_user(data.id_, arg.chat_id)
return sendMsg(arg.chat_id,arg.msg_id,'👤*¦* العضو » '..user_name..' \n🎫*¦* الايدي » (`'..data.id_..'`)\n🛠*¦* تم حظره \n✓️', "md")
...
if redis:get(LAITNEK..'welcome'..msg.to.id) then welcomex = "{ مفعل }" else welcomex = "{ معطل }" end
if redis:get(LAITNEK..'lock_id'..msg.to.id) then lock_idx = "{ مفعل }" else lock_idx = "{ معطل }" end
if redis:get(LAITNEK..'replay'..msg.to.id)  then replayx  = "{ مفعل }" else replayx  = "{ معطل }" end
if redis:get(LAITNEK..'lock_woring'..msg.to.id) then lock_woringx = "{ مفعل }" else lock_woringx  = "{ معطل }" end
list_settings = "*👮🏾¦*` اعدادات المجموعه :` \n\n📝¦ التعديل » "..(redis:get(LAITNEK..'lock_edit'..msg.to.id) or 'false').."\n🔗¦ الروابط » "..(redis:get(LAITNEK..'lock_link'..msg.to.id) or 'false').."\n#️⃣¦ التاك » "..(redis:get(LAITNEK..'lock_tag'..msg.to.id) or 'false').."\n©¦ المعرفات » "..(redis:get(LAITNEK..'lock_username'..msg.to.id) or 'false').."\n\n💱¦ التكرار » "..(redis:get(LAITNEK..'lock_flood'..msg.to.id) or 'false').."\n📑¦ الكلايش » "..(redis:get(LAITNEK..'lock_spam'..msg.to.id) or 'false').."\n🌐¦ الويب » "..(redis:get(LAITNEK..'lock_webpage'..msg.to.id) or 'false').."\n⚜️¦ الماركدوان » "..(redis:get(LAITNEK..'lock_markdown'..msg.to.id) or 'false').."\n🏌🏻¦ البوتات بالطرد » "..(redis:get(LAITNEK..'lock_bots_by_kick'..msg.to.id) or 'false').."\n🤖¦ البوتات » "..(redis:get(LAITNEK..'lock_bots'..msg.to.id) or 'false').."\n➕¦ عدد التكرار » "..(redis:get(LAITNEK..'num_msg_max'..msg.to.id) or 'false').."\n\n🎬¦` اعدادات الوسائط :`\n\n*¦* المتحركه » "..(redis:get(LAITNEK..'mute_gif'..msg.to.id) or 'false').."\n💭¦ الدردشه » "..(redis:get(LAITNEK..'mute_text'..msg.to.id) or 'false').."\n◽️¦ الانلاين » "..(redis:get(LAITNEK..'mute_inline'..msg.to.id) or 'false').."\n🎮¦ الالعاب » "..(redis:get(LAITNEK..'mute_game'..msg.to.id) or 'false').."\n🏞¦ الصور » "..(redis:get(LAITNEK..'mute_photo'..msg.to.id) or 'false').."\n🎥¦ الفيديو » "..(redis:get(LAITNEK..'mute_video'..msg.to.id) or 'false').."\n🎙¦ البصمات » "..(redis:get(LAITNEK..'mute_audio'..msg.to.id) or 'false').."\n\n🔉¦ الصوت » "..(redis:get(LAITNEK..'mute_voice'..msg.to.id) or 'false').."\n🎎¦ الملصقات » "..(redis:get(LAITNEK..'mute_sticker'..msg.to.id) or 'false').."\n📞¦ الجهات » "..(redis:get(LAITNEK..'mute_contact'..msg.to.id) or 'false').."\n💱¦ التوجيه » "..(redis:get(LAITNEK..'mute_forward'..msg.to.id) or 'false').."\n🌐¦ الموقع » "..(redis:get(LAITNEK..'mute_location'..msg.to.id) or 'false').."\n🗂¦ الملفات » "..(redis:get(LAITNEK..'mute_document'..msg.to.id) or 'false').."\n🔅¦ الاشعارات » "..(redis:get(LAITNEK..'mute_tgservice'..msg.to.id) or 'false').."\n🔒¦ الكيبورد » "..(redis:get(LAITNEK..'mute_keyboard'..msg.to.id) or 'false').."\n\n*⚒¦*` اعدادات اخرى : `\n*🙋🏼‍♂️¦* الترحيب » "..welcomex.."\n*💬¦*  الردود » "..replayx.."\n*🚸¦*  التحذير » "..lock_woringx.."\n*🎫¦* الايدي » "..lock_idx
